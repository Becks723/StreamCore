...
func (repo *iactiondb) switchToLike(ctx context.Context, tarType int, uid, tarId uint) error {
	return repo.db.Transaction(func(tx *gorm.DB) error {
		if err := tx.Model(&model.LikeRelationModel{}).
			Where("target_type = ? AND uid = ? AND target_id = ?", tarType, uid, tarId).
			Update("status", constants.LikeAction_Like).Error; err != nil {
			return err
		}
		if err := tx.Model(&model.LikeCountModel{}).
...
func (repo *iactiondb) switchToUnlike(ctx context.Context, tarType int, uid, tarId uint) error {
	return repo.db.Transaction(func(tx *gorm.DB) error {
		if err := tx.Model(&model.LikeRelationModel{}).
			Where("target_type = ? AND uid = ? AND target_id = ?", tarType, uid, tarId).
			Update("status", constants.LikeAction_Unlike).Error; err != nil {
			return err
		}
		if err := tx.Model(&model.LikeCountModel{}).
			Where("target_type = ? AND target_id = ?", tarType, tarId).
			Update("unlike_count", gorm.Expr("unlike_count + ?", 1)).Error; err != nil {
			return err
		}
		return nil
	})
